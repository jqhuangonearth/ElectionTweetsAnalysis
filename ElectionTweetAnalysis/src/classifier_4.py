...
Classification of campaigners, supporters and neutral users 
ONLY from hash tag information
'''
import cjson
...
class UserClassifier:
		...
		if not test_only:
			...
			try:
				for i in user_vectors_train:
					str = ''.join(i['tweets']) + ' ' + i['description']
					wds = str.split()
					new_str = ''
					for k in wds:
						if k.startswith("#"):
	...
	def cross_validate(self):
		...
		train_size = data_size/10;
		set_data = []
		start_idx = 0
		for j in range(10):
			set_data.append([self.data_ids[x] for x in range(start_idx, start_idx + train_size)])
			start_idx += train_size
		for j in range(10):
			self.test_ids = set_data[j]
			self.train_ids = []
			for i in range(10):
				if i == j: continue
				self.train_ids.extend(set_data[i])
