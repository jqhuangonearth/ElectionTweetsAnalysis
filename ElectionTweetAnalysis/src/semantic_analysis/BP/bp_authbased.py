...
@change: change the edge message direction A -> B becomes messge[B->A]
"""
...
def beliefprop(graph, _base_message_set={}, nodes_belief = {}, max_iterations=100, min_delta=0.0001):
    ...
    for i in range(max_iterations):
        ...
        for message in old_base_message_set.keys():
            ...
            sigma = 0.0
            # update "b" msg
            neighbors = graph.out_edges(message.split('#')[0]) # out edges
            ...
            for j in range(2): ## j from b to r
                ...
                if len(neighbors) == 0:
                    ...
                    for node in neighbors:
                        if node[1] != message.split("#")[1]:
                            product *= 1.0*old_base_message_set[node[1]+"#"+node[0]][j]
                    if len(nodes_belief[message.split("#")[0]]) > 1:
                        if j == 0:
    ...
    for node in nodes_belief:
        ...
        product = 1.0
        for n in neighbors:
            product *= base_message_set[n[1]+"#"+node][0]
        if len(nodes_belief[node]) > 1:
            b_b = nodes_belief[node][0]*product
        product = 1.0
        for n in neighbors:
            product *= base_message_set[n[1]+"#"+node][1]
        if len(nodes_belief[node]) > 1:
...
class bp:
    ...
    def cross_validation(self, iter = 10, fold = 10):
        ...
        """
        f = open("../results/results_fscore_accuracy_feb27_bp/bp_result_dj_%dfold.csv" %fold,"w")
        f.write("authority_based\n")
    ...
    def initialize_messages(self, G):
        ...
        edges = G.edges()
        for edge in edges:
            edge_messages.update({edge[1]+"#"+edge[0] : [1.0, 1.0]})
