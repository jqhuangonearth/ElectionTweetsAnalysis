...
class UserClassifier:
		...
		for line in f:
			...
			if len(line) >= 2:
				if int(line[1]) > 15 and line[2] == 's':
					...
					pass
					#self.data_ids.append(line[0])
				elif int(line[1]) > 0 and line[2] == 'n':
					self.data_ids.append(line[0])
					count_2 += 1
		print count, count_2
		print self.data_ids
	...
	def get_X_y(self, test_only=False):
		if not test_only:
			...
			self.vectorizer = TfidfVectorizer(min_df=1, max_features=100, stop_words='english')
			X_train = self.vectorizer.fit_transform(texts)
			#print 'num text features: ', len(self.vectorizer.get_feature_names())
			X_tr_t = X_train.toarray()
			### TFIDF vectors ####
		...
		if test_only:
			...
			return {'train': {'X': X_tr, 'Xt' : X_tr_t, 'y': y_tr}, 'test': {'X': X_te, 'Xt' : X_te_t,'y': y_te}}
	...
	def train(self):
		### decision tree model on non-text feature
		...
		out.close()
		### naive bayes model on non-text feature
		self.clf_2 = GaussianNB()
		self.clf_2.fit(self.train_data['X'], self.train_data['y'])
		### naive bayes model on tfidf text feature
		self.clf_3 = GaussianNB()
		self.clf_3.fit(self.train_data['Xt'], self.train_data['y'])
		
	
	def prediction(self):
		y_predicted = self.clf.predict(self.test_data['X'])
		y_predicted_2 = self.clf_2.predict(self.test_data['X'])
		y_predicted_3 = self.clf_3.predict(self.test_data['Xt'])
		...
		for i in range(0, len(self.test_data['y'])):
			if self.test_data['y'][i] != y_predicted[i]:
				print self.ID_te[i], self.test_data['y'][i], y_predicted[i]
		print '\t'
		'''
		...
		score = metrics.f1_score(self.test_data['y'], y_predicted_2)
		print "f1-score:   %0.3f" % score
		print metrics.classification_report(self.test_data['y'], y_predicted_2, target_names=['campaigners','neutral'])
		print "classification report-naive bayes (tfidf text feature):"
		score = metrics.f1_score(self.test_data['y'], y_predicted_3)
		print "f1-score:   %0.3f" % score
		print metrics.classification_report(self.test_data['y'], y_predicted_3, target_names=['campaigners','neutral'])
...
def main():
	uc = UserClassifier('user_data/labelled_new.txt', 50)
	#uc.initialize_data()
	#uc.train()
	#uc.prediction()
	uc.cross_validate()
